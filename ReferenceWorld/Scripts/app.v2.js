...
!function ($) {
    $(function () {
        ...
        function addMsg($msg) {
            var iCount = GetMessages();
            var $el = $('.nav-user'), $n = $('.count:first', $el), $v = parseInt($n.text());
            $('.count', $el).fadeOut().fadeIn().text(iCount);
            $($msg).hide().prependTo($el.find('.list-group')).slideDown().css('display', 'block');
            $("#noMessage").text(iCount);
        }
        var $msgText = '<a class="list-group-item" href="/Member/Message/' + $("#hidLoginUserGuid").val() + '"> <span class="badge bg-success media-body block m-b-none" id="noMessage">0</span> <i class="fa fa-comment icon-muted"></i> Messages </a>';
        var $msg1 = '<a href="#" class="media list-group-item">' + '<span class="pull-left thumb-sm text-center">' + '<i class="fa fa-envelope-o fa-2x text-success"></i>' + '</span>' + '<span class="media-body block m-b-none">' + 'Sophi sent you a email<br>' + '<small class="text-muted">1 minutes ago</small>' + '</span>' + '</a>';
        setTimeout(function () {
            addMsg($msgText);
        }, 1500);
        ...
        function GetMessages() {
            var userGuid = $("#hidLoginUserGuid").val();
            var iCount = 0;
            $.ajax({
                url: "/Member/GetMessages",async:false,
